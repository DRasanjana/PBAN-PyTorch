...
from layers import DynamicLSTM, SqueezeEmbedding, SoftAttention, Attention
...
class PBAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        alpha = F.softmax(
            F.tanh(torch.add(torch.bmm(torch.matmul(h_t, self.weight_m), torch.transpose(h_x, 1, 2)), self.bias_m)),
            dim=1)
        ...
        gamma = F.softmax(F.tanh(
            torch.add(torch.bmm(torch.matmul(h_x_pool, self.weight_n), torch.transpose(h_t, 1, 2)), self.bias_n)),
                          dim=1)
